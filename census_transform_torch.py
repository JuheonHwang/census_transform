...
def disparity_interp(disp_L, disp_R):
    ...
    R2L_y_grid = torch.clamp(grid_y.to(device) - disp_L, min=0, max=disp_L.size(2)-1).type(torch.cuda.DoubleTensor)
    L2R_y_grid = torch.clamp(grid_y.to(device) + disp_R, min=0, max=disp_L.size(2)-1).type(torch.cuda.DoubleTensor)
